...
knitr::kable(data.frame(gridsearch_data), caption = "Table 2.1 - R2 Scores For Various Alpha Values")
...
knitr::kable(data.frame(gridsearch_data), caption = "Table 2.2 - R2 Scores For Various Alpha Values")
...
### Results & Discussion
Here, we evaluated the best model we found on the test data. The results can be seen in the table below.
...
Overall, the model provides an acceptable estimate within the range of 0 to 200,000. However, it performs poorly when trying to predict higher values (>500,000) and thus could be improved upon in future updates.
...
```{r Table 4.1, echo = FALSE, fig.width = 5, fig.height = 4, out.width = "50%"}
pos_coef <- read.csv('../results/tables/positive_coefficients_ridge.csv')
knitr::kable(data.frame(pos_coef), caption = "Table 4.1 - Ten most positive coefficients")
...
```{r Table 4.2, echo = FALSE, fig.width = 5, fig.height = 4, out.width = "50%"}

neg_coef <- read.csv('../results/tables/negative_coefficients_ridge.csv')
knitr::kable(data.frame(neg_coef), caption = "Table 4.2 - Ten most negative coefficients")
...
knitr::kable(data.frame(neg_coef), caption = "Table 5 - Feature importance comparison")

```
<!-- **(include some text about random forest coefficient values being incomparable between the two)** -->
Note that the feature importance value is incomparable between the two models since the random forest model is not linear.
